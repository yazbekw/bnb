...
class FuturesTradingBot:
    ...
    def __init__(self):
        ...
        if not all([self.api_key, self.api_secret]):
            raise ValueValue("مفاتيح Binance مطلوبة")

        # استخدام API بإعدادات بسيطة للتداول الفعلي
        try:
        ...
        # العملات المعدلة
        self.symbols = ["ETHUSDT", "BNBUSDT", "XRPUSDT", "SOLUSDT", "DOGEUSDT"]
        
        # تخصيص Stop Loss و Take Profit لكل عملة
        self.symbol_settings = {
            "ETHUSDT": {'stop_loss_pct': 1.0, 'take_profit_pct': 1.5},
            "BNBUSDT": {'stop_loss_pct': 1.0, 'take_profit_pct': 1.5},
            "XRPUSDT": {'stop_loss_pct': 1.0, 'take_profit_pct': 2.0},
            "SOLUSDT": {'stop_loss_pct': 1.0, 'take_profit_pct': 2.5},
            "DOGEUSDT": {'stop_loss_pct': 1.0, 'take_profit_pct': 3.0},
        }
        
        self.active_trades = {}  # للعقود الآجلة
        self.active_spot_trades = {}  # للتداول الفوري
                ...
                if symbol in self.symbols:
                    ...
                    if quantity != 0:
                        ...
                        trade_data = {
                            ...
                            'trade_type': 'futures',
                            'highest_price': entry_price if side == 'LONG' else entry_price,  # لـ Trailing
                            'lowest_price': entry_price if side == 'SHORT' else entry_price,  # لـ Trailing
                            'trail_started': False
                        }
    ...
    def load_existing_spot_trades(self):
        try:
            ...
            for balance in balances:
                ...
                if free > 0:
                    # للبساطة، نفترض أن أي رصيد >0 هو صفقة مفتوحة، لكن يمكن تحسين
                    # لكن للتداول، نحتاج إلى تتبع الشراء
                    # هنا، للبدء، لا نحمل، أو نضيف منطق
                    # للآن، نفترض لا صفقات مفتوحة في الفوري عند البدء، أو أضف إذا لزم
                    pass
    ...
    def get_futures_precision(self, symbol):
        """جلب دقة الكمية والحد الأدنى للعقود الآجلة"""
        try:
    ...
    def get_spot_precision(self, symbol):
        """جلب دقة الكمية والحد الأدنى للتداول الفوري"""
        try:
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength):
        try:
            if len(self.active_trades) >= self.TRADING_SETTINGS['max_active_trades']:
                logger.info(f"⏸️ الحد الأقصى للصفقات العقود ({self.TRADING_SETTINGS['max_active_trades']})")
                return False

            should_trade, analysis, dir = self.analyze_symbol(symbol)
            if not should_trade or dir != direction:
                return False
            ...
            # تعديل الحجم إذا كان أقل من الحد الأدنى
            ...
            if order['status'] == 'FILLED':
                ...
                trade_data = {
                    ...
                    'last_notification': datetime.now(damascus_tz),
                    'trade_type': 'futures',
                    'highest_price': avg_price if direction == 'LONG' else avg_price,  # لـ Trailing
                    'lowest_price': avg_price if direction == 'SHORT' else avg_price,  # لـ Trailing
                    'trail_started': False
                }
    ...
    def execute_spot_trade(self, symbol, signal_strength):
        """تنفيذ صفقة فورية (LONG فقط)"""
        try:
            if len(self.active_spot_trades) >= self.TRADING_SETTINGS['max_active_trades']:
                logger.info(f"⏸️ الحد الأقصى للصفقات الفورية ({self.TRADING_SETTINGS['max_active_trades']})")
                return False
            ...
            if quantity <= 0:
                logger.error(f"❌ كمية غير صالحة للفوري: {quantity}")
                return False

            # تنفيذ الشراء (LONG فقط)
            ...
            if order['status'] == 'FILLED':
                ...
                trade_data = {
                    ...
                    'last_notification': datetime.now(damascus_tz),
                    'trade_type': 'spot',
                    'highest_price': avg_price,  # لـ Trailing
                    'trail_started': False
                }
        ...
        for symbol, trade in list(self.active_trades.items()):
            try:
                ...
                if trade['side'] == 'LONG':
                    ...
                    # تحديث أعلى سعر لـ Trailing
                    if current_price > trade['highest_price']:
                        trade['highest_price'] = current_price
                    # بدء Trailing إذا وصل الربح إلى trigger
                    if not trade['trail_started'] and pnl_percent >= self.TRADING_SETTINGS['trail_trigger_pct']:
                        ...
                        # إغلاق إذا انخفض السعر تحت Trailing
                    ...
                    if not trade['trail_started'] and pnl_percent >= self.TRADING_SETTINGS['trail_trigger_pct']:
                        ...
                        if new_trail < trade['trail_price']:
                            trade['trail_price'] = new_trail
                        # إغلاق إذا ارتفع السعر فوق Trailing
                        if current_price >= trade['trail_price']:
                            self.close_futures_trade(symbol, current_price, 'Trailing Stop')
                            continue
    ...
    def manage_spot_trades(self):
        ...
        for symbol, trade in list(self.active_spot_trades.items()):
            try:
                ...
                pnl_percent = ((current_price - trade['entry_price']) / trade['entry_price']) * 100  # LONG فقط
                # تحديث أعلى سعر لـ Trailing
                if current_price > trade['highest_price']:
                    trade['highest_price'] = current_price
                # بدء Trailing
                ...
                # جلب الإعدادات المخصصة للعملة (نفسها للفوري)
                stop_loss_pct = self.symbol_settings[symbol]['stop_loss_pct']
                take_profit_pct = self.symbol_settings[symbol]['take_profit_pct']
        ...
        try:
            ...
            for symbol in self.symbols:
                ...
                if should_trade:
                    ...
                    # فتح في العقود الآجلة
                    if symbol not in self.active_trades:
                        self.execute_futures_trade(symbol, direction, analysis['signal_strength'])
                    
                    # فتح في الفوري إذا كانت LONG فقط
                    if direction == 'LONG' and symbol not in self.active_spot_trades:
                        self.execute_spot_trade(symbol, analysis['signal_strength'])
                    
                    break  # صفقة واحدة per scan
