            ...
            for attempt in range(retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning("⏳ Rate limited, retrying after {retry_after}s")
...
class FuturesTradingBot:
    ...
    def __init__(self):
        ...
        self.symbols = ["SOLUSDT", "ETHUSDT", "BNBUSDT", "XRPUSDT", "ADAUSDT"]  # Updated symbols
        self.verify_symbols_availability()
    
        self.symbol_settings = {
            "SOLUSDT": {'stop_loss_pct': 2.5, 'take_profit_pct': 6.0},
            "ETHUSDT": {'stop_loss_pct': 3.0, 'take_profit_pct': 7.5},
            "BNBUSDT": {'stop_loss_pct': 2.5, 'take_profit_pct': 6.0},
            "XRPUSDT": {'stop_loss_pct': 2.0, 'take_profit_pct': 5.0},
            "ADAUSDT": {'stop_loss_pct': 3.0, 'take_profit_pct': 7.5},
        }
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength, analysis):
        try:
            ...
            # حساب الرافعة الديناميكية بناءً على ATR
            atr = analysis['atr']
            leverage = min(max_leverage, max(min_leverage, 5.0 / (atr / current_price)))
            leverage = round(leverage)  # Binance يتطلب leverage integer
        ...
        for symbol, trade in list(self.active_trades.items()):
            try:
                ...
                if data is not None and len(data) >= 200:
                    ...
                    atr = latest['atr']
                    if trade['side'] == 'LONG':
                        reverse_signal = (latest['sma50'] < latest['sma200'] and previous['sma50'] >= previous['sma200'])  # Death Cross
                        if reverse_signal:
                            self.close_futures_trade(symbol, current_price, 'Reverse Signal (Death Cross)')
                            continue
                    elif trade['side'] == 'SHORT':
                        reverse_signal = (latest['sma50'] > latest['sma200'] and previous['sma50'] <= previous['sma200'])  # Golden Cross
                        if reverse_signal:
                            self.close_futures_trade(symbol, current_price, 'Reverse Signal (Golden Cross)')
                            continue

                # تخصيص Stop-Loss وTake-Profit بناءً على ATR
                atr_multiplier_sl = 1.5  # مضاعف Stop-Loss
                atr_multiplier_tp = 3.0  # مضاعف Take-Profit
                stop_loss_price = trade['entry_price'] * (1 - atr_multiplier_sl * atr / trade['entry_price']) if trade['side'] == 'LONG' else trade['entry_price'] * (1 + atr_multiplier_sl * atr / trade['entry_price'])
                take_profit_price = trade['entry_price'] * (1 + atr_multiplier_tp * atr / trade['entry_price']) if trade['side'] == 'LONG' else trade['entry_price'] * (1 - atr_multiplier_tp * atr / trade['entry_price'])

                stop_loss_pct = abs((stop_loss_price - trade['entry_price']) / trade['entry_price']) * 100
                take_profit_pct = abs((take_profit_price - trade['entry_price']) / trade['entry_price']) * 100

                settings = self.symbol_settings.get(symbol, {'stop_loss_pct': stop_loss_pct, 'take_profit_pct': take_profit_pct})
                ...
                if trade['side'] == 'LONG':
                    if pnl_percent <= -settings['stop_loss_pct']:
                        self.close_futures_trade(symbol, current_price, 'Stop Loss')
                    elif pnl_percent >= settings['take_profit_pct']:
        ...
        try:
            ...
            try:
                ...
                if order['status'] == 'FILLED':
                    ...
                    pnl_usd = pnl_percent * quantity * trade['entry_price'] / 100 * trade['leverage']
    ...
    def scan_market(self):
        if len(self.active_trades) >= self.TRADING_SETTINGS['max_active_trades']:
            logger.info(f"⏸️ الحد الأقصى للصفقات النشطة ({self.TRADING_SETTINGS['max_active_trades']})، تخطي فحص السوق")
            if self.notifier:
                self.notifier.send_message(
                    f"⏸️ <b>تخطي فحص السوق</b>\nالسبب: الحد الأقصى للصفقات النشطة ({len(self.active_trades)} / {self.TRADING_SETTINGS['max_active_trades']})\nالوقت: {datetime.now(damascus_tz).strftime('%Y-%m-%d %H:%M:%S')}",
                    'skip_scan_max_trades'
                )
            return
