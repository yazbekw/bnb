    ...
    def scan_market(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                signal_summary[symbol] = "خطأ في الفحص"
                if self.notifier:
                    self.notifier.send_message(
        ...
        if self.notifier:
            ...
            for sym, strength in signal_summary.items():
                summary_message += f"{sym}: قوة الإشارة {strength}\n"
            summary_message += f"\nالوقت: {datetime.now(damascus_tz).strftime('%Y-%m-%d %H:%M:%S')}"
