...
class FuturesTradingBot:
    ...
    def can_open_trade(self, symbol):
        ...
        if symbol in self.active_trades:
            reasons.append(f"صفقة نشطة موجودة مسبقاً لـ {symbol}")
            
        # التحقق من الرصيد المتاح
        if self.symbol_balances.get(symbol, 0) < 5:
            reasons.append(f"رصيد غير كافي: ${self.symbol_balances.get(symbol, 0):.2f}")
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength, analysis):
        try:
            ...
            if not can_trade:
                ...
                self.send_trade_signal_notification(symbol, direction, analysis, False, reasons)
                return False
            ...
            if quantity <= 0 or quantity < precision_info['min_qty']:
                logger.error(f"❌ الكمية غير صالحة لـ {symbol}: {quantity}")
                return False
            ...
            logger.info(f"💰 تنفيذ أمر {symbol}: {direction} - كمية: {quantity} - رافعة: {leverage}x")
            ...
            if order['status'] == 'FILLED':
                ...
                self.performance_reporter.record_trade_open(symbol, direction, avg_price, trade_size_usd)
                ...
                logger.error(f"❌ فشل تنفيذ الأمر لـ {symbol}: {order}")
                return False
        ...
        try:
            ...
            for symbol, trade in list(self.active_trades.items()):
                try:
                    ...
                    pnl_usd = (pnl_percent / 100) * trade['trade_size_usd']
    ...
    def close_trade(self, symbol, trade, reason, pnl_percent, pnl_usd):
        ...
        try:
            ...
            if order['status'] == 'FILLED':
                # تحديث الرصيد
                self.symbol_balances[symbol] += trade['trade_size_usd'] + pnl_usd
...
def main():
    try:
        ...
        if hasattr(bot, 'notifier') and bot.notifier:
            bot.notifier.send_message(f"❌ فشل تشغيل البوت: {e}", 'error')
