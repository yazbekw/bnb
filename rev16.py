...
class FuturesTradingBot:
    ...
    def analyze_symbol(self, symbol):
        try:
            ...
            if buy_signal:
                ...
                signal_strength += min(30, max(10, 20 + ema_diff * 5))  # من 10 إلى 30 بناءً على قوة التقاطع
            if sell_signal:
                ema_diff = (latest['ema13'] - latest['ema5']) / latest['ema13'] * 100
                signal_strength += min(30, max(10, 20 + ema_diff * 5))
            ...
            if rsi_buy_ok and buy_signal:
                ...
                signal_strength += min(20, max(5, rsi_strength))  # أقوى عند RSI أقرب إلى rsi_oversold
            if rsi_sell_ok and sell_signal:
                rsi_strength = (latest['rsi'] - (self.TRADING_SETTINGS['rsi_overbought'] - 10)) / (self.TRADING_SETTINGS['rsi_overbought'] - (self.TRADING_SETTINGS['rsi_overbought'] - 10)) * 20
                signal_strength += min(20, max(5, rsi_strength))

            # MACD
            if macd_buy_signal and macd_above_zero:
                macd_strength = abs(latest['macd_histogram']) / max(0.001, abs(latest['macd'])) * 25
                signal_strength += min(25, max(10, macd_strength))
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength, analysis):
        try:
            ...
            # عامل Volume: إذا كان الحجم الحالي أكبر من المتوسط، زد الحجم بنسبة تصل إلى 20%
            volume_factor = min(0.2, (analysis['volume'] / analysis['volume_avg'] - 1) if analysis['volume_avg'] > 0 else 0)
            
            # عامل ATR: إذا كان التقلب (ATR) منخفضًا، زد الحجم (افتراضيًا، ATR منخفض يعني سوق أكثر استقرارًا، يمكن زيادة الحجم بنسبة تصل إلى 20%)
            # نفترض ATR طبيعي حوالي 1% من السعر، لذا إذا كان ATR < 0.01 * price، زد
            atr_normalized = analysis['atr'] / latest['close'] if 'close' in analysis else 0
            atr_factor = min(0.2, max(0, 0.2 - atr_normalized * 10))  # أقل ATR، أعلى factor
