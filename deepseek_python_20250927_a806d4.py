...
class FuturesTradingBot:
    ...
    def execute_spot_trade(self, symbol, signal_strength, analysis):
        try:
            ...
            if analysis['signal_strength'] < 70 or analysis['direction'] != 'LONG':
                logger.info(f"❌ إشارة غير صالحة لـ {symbol}: القوة {analysis['signal_strength']}%, الاتجاه {analysis['direction']}")
                return False
            ...
            if order['status'] == 'FILLED':
                ...
                if self.notifier:
                    self.notifier.send_message(
                        f"🚀 <b>فتح صفقة فورية جديدة مع MACD</b>\n"
                        f"العملة: {symbol}\n"
                        f"الاتجاه: LONG\n"
                        f"السعر: ${avg_price:.4f}\n"
                        f"الكمية: {quantity:.6f}\n"
    ...
    def manage_futures_trades(self):
        ...
        for symbol, trade in list(self.active_trades.items()):
            try:
                ...
                if minutes_since_update >= 10 and self.notifier:
                    self.notifier.send_message(
    ...
    def scan_opportunities(self):
        try:
            logger.info("🔍 مسح الفرص المتاحة مع MACD...")
            
            for symbol in self.symbols:
                should_trade, analysis, direction = self.analyze_symbol(symbol)
                
                if should_trade:
                    logger.info(f"🎯 إشارة {direction} معززة لـ {symbol} (قوة: {analysis['signal_strength']}%)")
                    
                    if self.notifier:
                        self.notifier.send_message(
                            f"🎯 <b>إشارة {direction} معززة بـ MACD</b>\n"
                            f"العملة: {symbol}\n"
                            ...
                            f"RSI: {analysis['rsi']:.1f}\n"
                            f"MACD: {analysis['macd']:.4f}\n"
                            f"إشارة MACD: {analysis['macd_signal']:.4f}\n"
    ...
    def run_bot(self):
        logger.info("🚀 بدء تشغيل بوت العقود الآجلة والفوري مع MACD")
        ...
        logger.info("✅ البوت يعمل الآن - في انتظار الإشارات المعززة بـ MACD...")
...
if __name__ == "__main__":
    ...
    print("🚀 بدء تشغيل بوت العقود الآجلة مع MACD...")
    main()