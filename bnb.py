...
import pytz
from cachetools import TTLCache  # إضافة للتخزين المؤقت
from flask_limiter import Limiter  # إضافة لحد المعدلات
...
class MomentumHunterBot:
    ...
    def __init__(self, dry_run=False):
        self.dry_run = dry_run  # وضع المحاكاة
        ...
        self.cache = TTLCache(maxsize=1000, ttl=300)  # تخزين مؤقت لـ 5 دقائق
    ...
    def get_all_trading_symbols(self):
        try:
            ...
            return ["BTCUSDT", "ETHUSDT"]  # قائمة احتياطية
    ...
    def get_multiple_tickers(self, symbols):
        try:
            tickers = self.safe_binance_request(self.client.get_ticker)
            return {t['symbol']: t for t in tickers if t['symbol'] in symbols}
        except Exception as e:
    ...
    def calculate_technical_indicators(self, data):
        try:
            ...
            # المتوسطات المتحركة بطريقة تدريجية
    ...
    def find_best_opportunities(self):
        ...
        def process_symbol(symbol):
            try:
                ...
                ticker = self.get_multiple_tickers([symbol]).get(symbol)
                if not ticker:
                    return None
    ...
    def backtest_strategy(self, symbol, start_date, end_date):
        try:
            # تحميل البيانات التاريخية (افتراضيًا، استخدم تاريخ البدء والنهاية)
            # هنا، يمكن توسيع المنطق لجلب البيانات من Binance أو مصدر آخر
            data = self.get_historical_data(symbol, '15m', 1000)  # مثال، قم بتعديله للتواريخ
            trades = []
            for i in range(50, len(data), 15):  # كل 15 شمعة
                subset = data.iloc[:i]
                score, details = self.calculate_momentum_score(symbol)  # استخدم subset
                if score >= self.momentum_score_threshold:
                    # محاكاة الدخول والخروج
                    entry_price = subset.iloc[-1]['close']
                    # افتراض خروج بعد 10 شمعات
                    if i + 10 < len(data):
