...
class CryptoBot:
    ...
    def check_and_notify_weak_signals(self, buy_signal, sell_signal):
        """التحقق وإرسال إشعارات للإشارات الضعيفة والمتوسطة"""
        try:
            current_time = datetime.now()
            
            # إشعار الشراء الضعيف (20-25%)
            if 20 <= buy_signal < self.BASELINE_BUY_THRESHOLD:
                last_notified = self.last_signal_notification.get('buy_weak')
                if not last_notified or (current_time - last_notified).total_seconds() >= 900:  # 15 دقيقة
                    self.send_signal_notification('buy', buy_signal, 'ضعيفة')
                    self.last_signal_notification['buy_weak'] = current_time
            
            # إشعار البيع الضعيف (20-25%)
            if 20 <= sell_signal < self.SELL_THRESHOLD:
                last_notified = self.last_signal_notification.get('sell_weak')
                if not last_notified or (current_time - last_notified).total_seconds() >= 900:
                    self.send_signal_notification('sell', sell_signal, 'ضعيفة')
                    self.last_signal_notification['sell_weak'] = current_time
            
            # إشعار الشراء المتوسط (25-50%)
            if 25 <= buy_signal < 50:
                last_notified = self.last_signal_notification.get('buy_medium')
                if not last_notified or (current_time - last_notified).total_seconds() >= 600:  # 10 دقائق
                    self.send_signal_notification('buy', buy_signal, 'متوسطة')
                    self.last_signal_notification['buy_medium'] = current_time
            
            # إشعار البيع المتوسط (25-50%)
            if 25 <= sell_signal < 50:
                last_notified = self.last_signal_notification.get('sell_medium')
                if not last_notified or (current_time - last_notified).total_seconds() >= 600:
                    self.send_signal_notification('sell', sell_signal, 'متوسطة')
                    self.last_signal_notification['sell_medium'] = current_time
                    
        except Exception as e:
            logger.error(f"خطأ في إشعارات الإشارات الضعيفة لـ {self.symbol}: {e}")

    def send_signal_notification(self, signal_type, strength, strength_level):
        """إرسال إشعار بالإشارة"""
        try:
            emoji = "🟢" if signal_type == 'buy' else "🔴"
            
            if strength_level == 'ضعيفة':
                level_emoji = "⚪"
            elif strength_level == 'متوسطة':
                level_emoji = "🔵"
            else:
                level_emoji = "🟡"
            
            message = f"{emoji} **إشارة {signal_type} {strength_level}** {level_emoji}\n"
            message += f"*رمز:* `{self.symbol}`\n"
            message += f"*القوة:* `{strength:.1f}%`\n"
            
            # إضافة تفاصيل المؤشرات
            contributions = self.last_buy_contributions if signal_type == 'buy' else self.last_sell_contributions
            if contributions:
                message += "\n📊 **مساهمات المؤشرات:**\n"
                for indicator, score in contributions.items():
                    indicator_emoji = "✅" if score > 8 else "⚠️" if score > 3 else "❌"
                    message += f"• {indicator}: {score:.1f}% {indicator_emoji}\n"
            
            message += f"\n⏰ الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            
            if self.notifier.send_message(message):
                logger.info(f"تم إرسال إشعار إشارة {strength_level} لـ {self.symbol}")
                return True
            return False
            
        except Exception as e:
            logger.error(f"خطأ في إرسال إشعار الإشارة لـ {self.symbol}: {e}")
            return False

    def execute_trading_strategy(self):
        """تنفيذ استراتيجية التداول مع الإشعارات المحسنة"""
        try:
            # جلب البيانات
            data = self.fetch_historical_data(self.symbol)
            ...
            # ✅ إرسال إشعارات للإشارات الضعيفة والمتوسطة
            self.check_and_notify_weak_signals(buy_signal_strength, sell_signal_strength)
...
class Multi_Crypto_Trading_Bot:
    def __init__(self):
        ...
        logger.info("تم تهيئة بوت التداول متعدد العملات بنجاح مع الإشعارات المحسنة")

    def send_hourly_status(self):
        """إرسال حالة البوت والأسعار كل ساعة"""
        try:
            current_time = datetime.now()
            current_hour = current_time.hour
            
            # التحقق إذا تم الإرسال هذه الساعة already
            if self.last_hourly_notification == current_hour:
                return False
                
            message = "⏰ **التحديث الساعي للبوت**\n\n"
            
            # معلومات الأسعار
            message += "💹 **أسعار العملات:**\n"
            for symbol in self.bot_settings.keys():
            ...
            for symbol, bot in self.bots.items():
                orders_count = bot.get_algo_orders_count()
                balance = bot.get_symbol_balance()
                message += f"• {symbol}: {orders_count} أوامر نشطة | الرصيد: {balance:.4f}\n"
            
            # الأداء اليومي
            performance = self.get_overall_performance()
            daily_pnl = performance['overall']['total_pnl']
            pnl_emoji = "📈" if daily_pnl > 0 else "📉" if daily_pnl < 0 else "➡️"
            message += f"\n{pnl_emoji} **الأداء اليومي:** ${daily_pnl:.2f}"
            
            message += f"\n\n⏰ الوقت: {current_time.strftime('%Y-%m-%d %H:%M:%S')}"
            
            if self.notifier.send_message(message):
                self.last_hourly_notification = current_hour
                logger.info("تم إرسال الإشعار الساعي بنجاح")
                return True
            return False
                
        except Exception as e:
            logger.error(f"خطأ في إرسال التحديث الساعي: {e}")
            return False

    def get_current_price(self, symbol):
        """الحصول على السعر الحالي لأي عملة"""
        try:
...
def main():
    """الدالة الرئيسية مع الإشعارات الساعية"""
    try:
        ...
        logger.info("بدأ البوت الرئيسي مع الإشعارات الساعية")
        ...
        while True:
            try:
                ...
                # ✅ تشغيل دورة التداول كل 15 دقيقة
                if (current_time - last_trading_cycle).total_seconds() >= 900:
                    bot.run_trading_cycle()
                    last_trading_cycle = current_time
                
                # ✅ التحقق من الإشعار الساعي كل 5 دقائق
                if (current_time - last_hourly_check).total_seconds() >= 300:
                    bot.send_hourly_status()
                    last_hourly_check = current_time
                
                # انتظار 60 ثانية قبل التكرار
                time.sleep(60)
        ...
        if 'bot' in locals():
            bot.notifier.send_message("⛔ إيقاف البوت - نظام الإشعارات المحسن")
    except Exception as e:
