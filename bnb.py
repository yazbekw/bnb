...
class MomentumHunterBot:
    ...
    def calculate_adx(self, df, period=14):
        """حساب ADX بناءً على الكود الأول"""
    ...
    def calculate_momentum_score(self, symbol):
        try:
            ...
            # 2. تقاطع المتوسطات (20 نقطة) مع نافذة زمنية كما في الكود الأول
            window = data.iloc[max(0, len(data)-4):]  # نافذة 3 شمعات سابقة + الحالية
    ...
    def calculate_position_size(self, opportunity, usdt_balance):
        """
        حساب حجم الصفقة بناءً على المخاطرة كما في الكود الأول
        """
        try:
            ...
            # حساب المخاطرة بناءً على النتيجة
            if score >= 80:
                risk_pct = 0.007  # مخاطرة أعلى للإشارات القوية
            ...
            stop_distance = atr * 2.5  # ATR multiplier
    ...
    def manage_active_trades(self):
        """إدارة الصفقات النشطة مع منع الإغلاق الفوري وإضافة إشارات البيع من الكود الأول"""
        for symbol, trade in list(self.active_trades.items()):
            try:
                ...
                # إضافة إشارات البيع من الكود الأول
                data = self.get_historical_data(symbol, '15m', 100)
                if data is None or len(data) < 50:
                    continue
                data = self.calculate_technical_indicators(data)
