...
class BNB_Trading_Bot:
    ...
    def add_trade_record(self, trade_type, quantity, price, trade_size, signal_strength, order_id=None, status="executed", profit_loss=0, exit_type=None):
    ...
    def update_trailing_stop(self, order_id, current_price):
        ...
        if order_id in self.active_trailing_stops:
            trailing_info = self.active_trailing_stops[order_id]
            entry_price = trailing_info['entry_price']
            initial_stop = trailing_info['initial_stop']
            activation_percent = trailing_info['activation_percent']
            trailing_percent = trailing_info['trailing_percent']
            
            # التحقق إذا تم تفعيل التريلينغ
            if current_price >= entry_price * (1 + activation_percent):
                # حساب التريلينغ ستوب الجديد
                new_stop = current_price * (1 - trailing_percent)
                
                # التأكد من أن التريلينغ لا ينخفض عن المستوى الأولي
                if new_stop > initial_stop and new_stop > trailing_info['current_stop']:
                    trailing_info['current_stop'] = new_stop
                    self.active_trailing_stops[order_id] = trailing_info
                    
                    # إرسال إشعار بالتحديث
                    self.send_notification(
                        f"📈 تحديث التريلينغ ستوب للأمر {order_id}\n"
                        f"السعر الحالي: ${current_price:.4f}\n"
                        f"وقف الخسارة الجديد: ${new_stop:.4f}\n"
                        f"المكاسب المحمية: {((current_price - new_stop) / (entry_price - initial_stop) * 100):.1f}%"
                    )
                    return new_stop
            
            return trailing_info['current_stop']
        return None
    ...
    def execute_real_trade(self, signal_type, signal_strength, current_price, stop_loss, take_profit):
        ...
        if not trading_enabled:
            self.send_notification(
                f"🛑 توقف التداول اليومي - وصل للحد الأقصى للخسارة\n"
                f"الخسارة اليومية: {daily_loss_pct:.2f}%\n"
                f"الحد المسموح: 2.00%\n"
                f"سيستأنف التداول غداً"
            )
            return False
        ...
        try:
            ...
            if signal_type == 'buy':
                ...
                # إعداد التريلينغ ستوب
                self.active_trailing_stops[order_id] = {
                    'entry_price': current_price,
                    'initial_stop': stop_loss,
                    'current_stop': stop_loss,
                    'activation_percent': 0.02,  # تفعيل عند تحقيق 2% ربح
                    'trailing_percent': 0.015,   # تريلينغ 1.5%
                    'quantity': quantity
                }
    ...
    def check_order_executions(self):
        """التحقق من تنفيذ أوامر الوقف وجني الأرباح"""
        try:
            open_orders = self.client.get_open_orders(symbol=self.symbol)
            order_ids = [order['orderId'] for order in open_orders]
            
            # التحقق من الأوامر التي تم إغلاقها
            for order_id in list(self.active_trailing_stops.keys()):
                if order_id not in order_ids:
                    # الأمر تم إغلاقه
                    trailing_info = self.active_trailing_stops.pop(order_id, None)
                    if trailing_info:
                        # البحث في تاريخ الصفقات لمعرفة نوع الخروج
                        for trade in self.trade_history:
                            if trade.get('order_id') == order_id and trade.get('exit_type'):
                                self.send_notification(
                                    f"✅ تم إغلاق الصفقة {order_id}\n"
                                    f"نوع الخروج: {trade['exit_type']}\n"
                                    f"الربح/الخسارة: ${trade.get('profit_loss', 0):.2f}\n"
                                    f"الحجم: ${trade.get('trade_size', 0):.2f}"
                                )
                                break
            
            # تحديث التريلينغ ستوب للأوامر النشطة
            ticker = self.client.get_symbol_ticker(symbol=self.symbol)
            current_price = float(ticker['price'])
            
            for order_id in self.active_trailing_stops.keys():
                self.update_trailing_stop(order_id, current_price)
                
        except Exception as e:
            logger.error(f"خطأ في التحقق من تنفيذ الأوامر: {e}")
        ...
        if signal_type in ['buy', 'sell']:
            ...
            if success:
                ...
                if signal_type == 'buy':
                    ...
                    msg += f"التريلينغ ستوب: مفعل (2% تفعيل، 1.5% تتبع)\n"
    ...
    def generate_signal_analysis(self, data, signal_type, signal_strength, order_status):
        ...
        analysis += f"ADX: {latest.get('adx', 0):.1f}\n"
    ...
    def send_performance_report(self):
        try:
            ...
            # التحقق من الحد اليومي للخسارة
            trading_enabled, daily_loss_pct = self.performance_analyzer.check_daily_loss_limit(total_balance)
            ...
            message += f"الخسارة اليومية: {daily_loss_pct:.2f}% / 2.00%\n"
            message += f"حالة التداول: {'✅ نشط' if trading_enabled else '🛑 موقوف'}\n"
            ...
            if 'total_trades' in report_12h and report_12h['total_trades'] > 0:
                ...
                if report_12h.get('exit_types'):
                    message += f"\nأنواع الخروج: {', '.join([f'{k}: {v}' for k, v in report_12h['exit_types'].items()])}"
    ...
    def run(self):
        ...
        while True:
            try:
                # التحقق من الحد اليومي للخسارة
                current_balance = self.get_real_balance()
                trading_enabled, _ = self.performance_analyzer.check_daily_loss_limit(current_balance)
