...
class APIRateLimiter:
    """محدد لمعدل طلبات API لتجنب الحظر"""
    
    def __init__(self):
        self.last_request_time = 0
        self.min_interval = 1.2  # ثانية بين الطلبات
    ...
    def wait_if_needed(self):
        """الانتظار إذا كانت الطلبات سريعة جداً"""
        ...
        # تحذير إذا اقتربنا من الحدود
        if self.request_count > 40:  # 40 طلب في الدقيقة
            logger.warning("⚠️ اقتراب من حد الطلبات في الدقيقة")
        if self.daily_requests > 8000:  # 8000 طلب في اليوم
            logger.warning("⚠️ اقتراب من حد الطلبات اليومي")

class PerformanceReporter:
    """كلاس محسن لتقارير أداء البوت مع حسابات دقيقة"""
    
    def __init__(self, bot_instance, notifier, rate_limiter):
...
class PriceManager:
    def __init__(self, symbols, client, rate_limiter):
    ...
    def can_update_prices(self):
        ...
        current_time = time.time()
        # إذا مر أكثر من 30 ثانية منذ آخر تحديث، السماح بالتحديث
        if current_time - self.last_bulk_update > self.update_interval:
            return True
        # إذا كان عدد التحديثات أقل من 50 في الدقائق الأخيرة
    ...
    def get_price(self, symbol):
        ...
        try:
            ...
            # إذا مر أكثر من 60 ثانية منذ آخر تحديث، محاولة التحديث
...
class FuturesTradingBot:
    ...
    TRADING_SETTINGS = {
        ...
        'max_active_trades': 3,
        'data_interval': '1h',  # زيادة الفترة لتقليل الطلبات
        'rescan_interval_minutes': 15,  # زيادة فترة المسح
        'price_update_interval': 5,  # زيادة فترة تحديث الأسعار
        'trade_timeout_hours': 8.0,
        'min_signal_conditions': 5,
        'atr_stop_loss_multiplier': 1.5,
        'atr_take_profit_multiplier': 3.0,
        'min_trade_duration_minutes': 60,
    ...
    def __init__(self):
        ...
        # تهيئة تقارير الأداء مع محدود المعدل
        self.performance_reporter = PerformanceReporter(self, self.notifier, self.rate_limiter)
        ...
        self.active_trades = {}
        self.price_manager = PriceManager(self.symbols, self.client, self.rate_limiter)
        ...
        logger.info("✅ تم تهيئة البوت بنجاح مع حماية من الحظر")

    def start_trade_sync(self):
        """بدء مزامنة الصفقات بشكل دوري"""
        def sync_thread():
            while True:
                try:
                    # التحقق من عدد الصفقات النشطة قبل المزامنة
                    if len(self.active_trades) >= 3:
                        time.sleep(180)  # انتظار أطول إذا كانت هناك صفقات نشطة
                        continue
                        
                    self.load_existing_trades()
                    time.sleep(120)  # مزامنة كل دقيقتين
                except Exception as e:
    ...
    def test_telegram_connection(self):
        ...
        if self.notifier:
            test_message = "🔊 <b>اختبار اتصال التلغرام</b>\n✅ البوت يعمل بشكل صحيح مع حماية من الحظر"
    ...
    def start_performance_reporting(self):
        ...
        if self.notifier:
            schedule.every(4).hours.do(self.send_performance_report)  # تقليل التكرار
            schedule.every().day.at("00:00").do(self.performance_reporter.reset_daily_stats)
            schedule.every(45).minutes.do(self.send_heartbeat)  # تقليل التكرار
    ...
    def send_startup_message(self):
        if self.notifier:
            try:
                message = (
                    "🚀 <b>بدء تشغيل بوت العقود الآجلة - النسخة الآمنة</b>\n\n"
                    f"📊 <b>الميزات الجديدة:</b>\n"
                    f"• نظام حماية من الحظر\n"
                    f"• معدل طلبات محدود (1.2 ثانية بين الطلبات)\n"
                    f"• تحديث أسعار كل 30 ثانية\n"
                    f"• مزامنة الصفقات كل دقيقتين\n"
                    f"• فترات انتظار أطول بين العمليات\n\n"
                    f"🕒 <b>وقت البدء:</b>\n"
    ...
    def get_historical_data(self, symbol, interval='1h', limit=50):  # تقليل الحد
        try:
    ...
    def analyze_symbol(self, symbol):
        try:
            ...
            # ✅ تحديد الإشارة
            min_score = 4.5  # نظام مرجح بدلاً من العد الثابت
    ...
    def should_accept_signal(self, symbol, direction, analysis):
        ...
        if abs(analysis['price_vs_sma20']) > 8:
    ...
    def run(self):
        """تشغيل البوت الرئيسي"""
        logger.info("🚀 بدء تشغيل بوت العقود الآجلة (النسخة الآمنة)...")
...
def main():
    try:
        # انتظار حتى انتهاء الحظر إذا كان هناك حظر
        current_time = int(time.time() * 1000)
        ban_until = 1759429604409  # الوقت من الخطأ
        
        if current_time < ban_until:
            wait_seconds = (ban_until - current_time) // 1000
            logger.info(f"⏳ الانتظار حتى انتهاء الحظر: {wait_seconds} ثانية...")
            time.sleep(wait_seconds + 10)  # انتظار إضافي 10 ثواني
