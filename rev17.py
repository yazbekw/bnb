...
class TelegramNotifier:
    ...
    def send_message(self, message, message_type='info', retries=3, delay=5):
        ...
        try:
            ...
            # تقليل القيد للإشعارات الهامة (مثل trade, error, extend) إلى 30 ثانية، وإزالة القيد للأخطاء
            min_interval = 0 if message_type.startswith('error') else 30 if message_type in ['trade_open_futures', 'trade_close_futures', 'extend_futures', 'signal_futures'] else 60
            if current_time - last_sent < min_interval and message_type != 'trade':
