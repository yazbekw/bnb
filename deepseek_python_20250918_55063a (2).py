...
class MomentumHunterBot:
    def __init__(self):
        ...
        # إعدادات التداول المتطورة والمعدلة
        self.symbols = self.get_all_trading_symbols()
        self.stable_coins = ['USDT', 'BUSD', 'USDC']
        self.min_daily_volume = 5000000  # زيادة إلى 5M USD حجم يومي
        self.min_trade_size = 100  # زيادة الحد الأدنى للصفقة إلى 100 USDT
        self.risk_per_trade = 2.0  # 2% مخاطرة لكل صفقة بدلاً من مبلغ ثابت
    ...
    def calculate_technical_indicators(self, data):
        try:
            ...
            # المتوسطات المتحركة باستخدام talib للحساب الدقيق
    ...
    def calculate_position_size(self, opportunity, usdt_balance):
        """
        حساب حجم الصفقة المتدرج مع مراعاة الحد الأدنى للربح بعد الرسوم
        """
        try:
            ...
            if score >= 90:
                ...
                risk_percentage = 0.12  # 12%
                risk_level = "قوية جداً 💪"
            elif score >= 80:
                risk_percentage = 0.10  # 10%
                risk_level = "قوية 👍"
            elif score >= 75:
                risk_percentage = 0.08  # 8%
                risk_level = "جيدة 🔄"
            else:
            ...
            # التأكد من أن الحجم لا يقل عن الحد الأدنى
            if base_position_size < self.min_trade_size:
                base_position_size = self.min_trade_size
            
            # حساب الحد الأقصى بناء على ATR
            atr_risk = current_price * 0.02  # 2% خطر
            atr_position_size = (usdt_balance * self.risk_per_trade / 100) / atr_risk
            
            # أخذ القيمة الأصغر
            position_size_usdt = min(base_position_size, atr_position_size * current_price)
            ...
            logger.info(f"📊 حجم الصفقة لـ {opportunity['symbol']}: "
                       f"${position_size_usdt:.2f} ({risk_percentage*100:.1f}%) - "
    ...
    def run_bot(self):
        ...
        if self.notifier:
            self.notifier.send_message("🚀 <b>بدء تشغيل البوت المحسن</b>\nتم تحسين استراتيجية التداول لمنع الخسائر", 'startup')
        ...
        while True:
            try:
                ...
                for opportunity in opportunities[:2]:  # الحد الأقصى لصفقتين في نفس الوقت
                    if len(self.active_trades) >= 3:  # الحد الأقصى 3 صفقات نشطة
                        break
                        
                    if opportunity['symbol'] not in self.active_trades:
                        self.execute_trade(opportunity)
                        time.sleep(2)  # فاصل بين الصفقات
...
def main():
    try:
        # بدء تطبيق Flask في thread منفصل
        ...
        if 'bot' in locals() and bot.notifier:
            bot.notifier.send_message(f"💥 <b>توقف البوت بسبب خطأ فادح</b>\n{str(e)}", 'critical_error')
