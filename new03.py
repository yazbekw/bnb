...
class FuturesTradingBot:
    ...
    def __init__(self):
        ...
        self.start_performance_reporting()  # بدء التقارير الدورية
    ...
    def send_startup_message(self):
        if self.notifier:
            try:
                message = (
                    "🚀 <b>بدء تشغيل بوت العقود الآجلة - مع التقارير</b>\n\n"
                    f"📊 <b>الميزات الجديدة:</b>\n"
                    f"• تقرير أداء كل 3 ساعات\n"
                    f"• إحصائيات مفصلة عن الصفقات\n"
                    f"• تتبع الأداء المالي\n"
                    f"• تحليل نسبة الربح والخسارة\n\n"
                    f"🕒 <b>وقت البدء:</b>\n"
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength, analysis):
        try:
            ...
            if order['status'] == 'FILLED':
                ...
                # تسجيل فتح الصفقة في تقارير الأداء
                if hasattr(self, 'performance_reporter'):
                    self.performance_reporter.record_trade_open(
                        symbol, direction, avg_price, trade_size_usd
                    )
    ...
    def close_futures_trade(self, symbol, current_price, reason):
        try:
            ...
            if order['status'] == 'FILLED':
                ...
                # تسجيل إغلاق الصفقة في تقارير الأداء
                if hasattr(self, 'performance_reporter'):
                    self.performance_reporter.record_trade_close(
                        symbol, exit_price, pnl_percent, pnl_usd, reason
                    )
                    # تحديث إحصائيات الرصيد
                    current_balance = sum(self.symbol_balances.values())
                    self.performance_reporter.update_balance_stats(current_balance)
