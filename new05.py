...
class FuturesTradingBot:
    ...
    def execute_futures_trade(self, symbol, direction, signal_strength, analysis):
        try:
            ...
            if not can_trade:
                ...
                if self.notifier:
                    error_message = (
                        ...
                        f"العملة: {symbol}\n"
                        f"الاتجاه: {direction}\n"
                        f"<b>السبب:</b> {', '.join(reasons)}\n"
                        f"الوقت: {datetime.now(damascus_tz).strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                    self.notifier.send_message(error_message, f'trade_error_{symbol}')
                return False
            ...
            # حساب الكمية الصحيح: quantity = (margin * leverage) / price
            ...
            if quantity <= 0 or quantity < precision_info['min_qty'] or notional_value < precision_info['min_notional']:
                raise Exception(f"الكمية غير صالحة: {quantity}, قيمة اسمية: {notional_value} < {precision_info['min_notional']}")
